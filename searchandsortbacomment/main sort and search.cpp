...
void Sort::bubble_sort(int arr[] , int size)
{
    ...
    for(int i=n-2 ; i>=0 ; i--)
    {
        for(int j=0 ; j<=i ; j++)
        {
            if(arr[j] > arr[j+1])//agar onsor feeli > onsor badi hast ja be ja kon
            {
                temp=arr[j];
                arr[j]=arr[j+1];
                arr[j+1]=temp;
...
void Sort::selection_sort(int arr[] , int size)
{
    ...
    for(int i=0 ; i<n-1 ; i++)
    {
        ...
        if(min_index != i)//agar min index motefavet az andis feli ast onsor ha ra ja be ja kon
        {
            tmp = arr[i];
            arr[i] = arr[min_index];
            arr[min_index] = tmp;
...
void Sort::insertion_sort(int arr[] , int size)
{
   int i,j,x;
    ...
    {
        x = arr[i];
        for(j=i ; j>0 ; j--)//agar osor feli > osor ghabli ast ja be ja kon
        {
            if(x>=arr[j-1])
                break;
            arr[j]=arr[j-1];
        }
        arr[j] = x;//onsor feli ra dar makan monaseb darj kon
    }
...
void merg(int arr[] ,int left , int mid ,int right)
{
    ...
    //copy kardan anasor zir arraye chap be arraye leftarr
    for(int i=0 ; i<mid-left+1 ; i++)
        leftarr[i]=arr[left+i];
    //copy kardan anasor zir arraye rast be arraye rightarr
    ...
    //edqam 2 zir arraye dar arraye asli(arr)
    ...
    //ezafe kardan baqi anasor zir array rast be arraye asili(arr)
    ...
    //ezafe kardan baqi anasor zir array chap be arraye asili(arr)
...
{
    int mid = (left+right)/2;//mohasebe miangin andis chap va rast
    
    //agar osor mored nazar peyda shavad
    if(arr[mid] == x)
        return "Find";
    //agar entehaye baze barabar bashad va onsor da miyane nabashad
    if(left == right && left==mid )
        return "Not Find";
    //agar onsor dar nime badi bashad
    if(arr[mid] < x)
    {
        left = mid +1;
        return BinarySearch(arr , left , right ,x);
    }
    //agar onsor da nime ghabli bashad
    else
    {
        right = mid-1;
        return BinarySearch(arr, left , right, x);
...
int main()
{
    ...
    int size =5;
    int arr[size];
    cout<<"Enter anasor arraye be tartib for searching: ";
    for (int i=0 ; i<size ; i++)
        cin>>arr[i];
